...
est_SzPop<-function(stratum,lengths,strata_dat,cpue_dat,lfreq_dat){
  ...
  SZPOP_M<-colSums(SZPOP_M_st)
  
  # Females
  SZPOP_F_st[is.na(SZPOP_F_st)] <- 0
  SZPOP_F<-colSums(SZPOP_F_st)
  
  # Unsexed
  SZPOP_U_st[is.na(SZPOP_U_st)] <- 0
  SZPOP_U<-colSums(SZPOP_U_st)
  
  # Set Size pop'n ests for age pop'n est script
  SzPop<-cbind(lengths,SZPOP_M,SZPOP_F,SZPOP_U)
  colnames(SzPop)<-c("LENGTH","MALES","FEMALES","UNSEXED")
